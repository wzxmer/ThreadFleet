...
- 电脑操控路由：在“设置 > Codex > MCP”查看并刷新当前执行主机的能力状态。Windows 原生界面固定使用 `windows-ui`，依赖现有登录态的网页使用 Chrome，隔离网页使用 Browser；后端不可用时会明确停止，不静默切换会话边界或尝试 Computer Use。Windows 本地执行主机默认在启动时检查 `windows-ui` 官方稳定版，只有用户再次确认后才会下载、验证并安装；非托管配置、远程执行主机和非 Windows 平台不会被修改。安装后需新建或重新连接 Codex 会话，ThreadFleet 不会强制结束已有会话。
