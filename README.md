...
- 每个工作区的会话列表首屏最多驻留 6 个任务，点击“加载更早会话”后每次再读取 6 个；收起工作区会释放追加页和子任务展开界面，同时保留当前、置顶、运行中及等待响应的任务锚点。
