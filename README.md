...
- 用量显示：左下角 Codex 用量可开关，支持已用/剩余额度切换；用量来源可选择本机 Codex 配置，或按“服务商 > 分组 > API 密钥”显式选择，两种选择都会同步当前执行身份，其中本机配置等同于设置中的“使用 Codex 默认配置”。第三方模型服务商可读取 Sub2 或 New API 用量，New API 可额外配置 Access Token 读取账户余额。日志不足以完整覆盖当天时会明确回退为累计消费；首页按小时、今日、近 7 天和本月汇总本机全部 Codex 会话，包含归档记录，并区分缓存读取和未缓存输入。
