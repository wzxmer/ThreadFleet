...
export function SessionManagerList(props: Props) {
  ...
  return (
    <div className="session-manager-list">
      {contextMenu && <SessionManagerContextMenu sessions={contextMenu.sessions} sources={props.sources} x={contextMenu.x} y={contextMenu.y} boundary={contextMenu.boundary} busy={props.loading || props.loadingMore || props.resumingKey !== null || props.archivingKeys.size > 0 || Boolean(props.deletingKeys?.size)} onClose={closeContextMenu} onResume={props.onResume} onDerive={(sessions) => { if (props.onDeriveSelected) props.onDeriveSelected(sessions); else sessions.forEach(props.onDerive); }} onArchive={(sessions) => { if (props.onArchiveSelected) props.onArchiveSelected(sessions); else sessions.forEach(props.onArchive); }} onPermanentDelete={props.onPermanentDelete ?? (() => {})} />}
